...
let fill_map len wid =
  let (t : path_tile) = {coor = 0, 0; length = 1000;prev = None} in
  let x = Array.make_matrix len wid t in
  co x;
  x
...
let rec update_frontier (f : ( tile * int) list) (tl : tile) (m : map) (pmap : path_map) =
  ...
  |h::t ->
    ...
    |(x,y) ->
      ...
      print_string (" Cost"^(string_of_int (curr+cost)));
...
let rec found_frontier (s : tile list) (tile : tile) (map : map) (pmap : path_map) =
  ...
  |h::t ->
    if check_adjacent h tile map then
      let found_tile = pmap.grid.(fst tile.coordinate).(snd tile.coordinate) in
